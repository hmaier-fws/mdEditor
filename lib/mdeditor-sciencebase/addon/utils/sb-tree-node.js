...
export default EmberObject.extend({
  ...
  isLoading: false,
  isExpanded: true,
  notSelectable: computed('isSelected', 'parentNode.isSelected', 'sbId', 'sbParentId',
    function () {
  ...
  sbParentIdObj: computed('_record', 'sbParentId', function () {
    let record = this._record;
  ...
  sbDate: computed('_record', 'sbId', function () {
    let record = this.findSbIdObject(this._record);
  ...
  addSbId(id) {
    ...
    set(record, path, (get(record, path) === undefined ? arr : get(record, path)));
  ...
  addSbParentId(id) {
    ...
    set(record, path, (get(record, path) === undefined ? {
      title: "Parent Metadata"
    } : get(record, path)));

    set(record, idPath, (get(record, idPath) === undefined ? arr : get(record, idPath)));
  ...
  addChildren(records) {
    let children = records.filter((itm) => {
        ...
        return parentIds.findBy('identifier', this.id);
      })
      .map(rec => {
  ...
  publish(jossoParam) {
    ...
    let data = {
      data: {
        ...
        community_id: record.get('sbParentId') || record.get(
          'parentNode.sbId') || defaultParent,
      ...
      .then(function (response) {
        ...
        if(response.id) {
          ...
          if(idObj) {
            ...
            set(idObj, 'authority', (idObj.authority === undefined ? {
              date: [],
              title: 'ScienceBase'
            } : idObj.authority));
            set(idObj, 'authority.title', (get(idObj, 'authority.title') === undefined ? 'ScienceBase' : get(idObj, 'authority.title')));
            set(idObj, 'authority.date', (get(idObj, 'authority.date') === undefined ? [] : get(idObj, 'authority.date')));

            get(idObj, 'authority.date')
              .pushObject({
                date: moment()
                  .toISOString(),
                dateType: 'published',
          ...
          return allSettled(record.children.filterBy('isSelected')
            .map((
              child) => {
        ...
        let error =
          `Error Publishing: ${response.status}: ${response.statusText}`;
